 ...
 **/
class MageTool_Tool_MageTheme_Provider_Package extends MageTool_Tool_MageTheme_Provider_Abstract
...
{    
     ...
     * @author Alistair Stead
     **/
    public function create($name, $area = 'frontend', $fileOfProfile = null)
    {
        $this->_bootstrap();
        ...
        $path = sprintf("%s/app/design/%s/%s", $path, $area, $this->name);

        if (file_exists($path)) {
            throw new MageTool_Tool_MageExtension_Provider_Exception(
                "A Package {$this->name} already exists in the {$this->area} area."
                );
        } else {
            try {
        ...
        $response->appendContent('Created Package at ' . $path, array('color' => 'green'));
        $response->appendContent('Note: ', array('separator' => true, 'color' => 'yellow'));
        $response->appendContent(
            'This command created a new Package, '
